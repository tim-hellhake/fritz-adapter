...
export class FritzButton extends FritzTemperatureSensor {
    ...
    public getButtons(): SubButton[] {
      return this.deviceInfo.buttons?.map((button) => {
        return {...button};
      }) ?? [];
    }
}
...
export class FritzBulb extends FritzDevice {
  ...
  public async setColor(color: SubColor): Promise<void> {
  ...
  public async setTemperature(temperature: ColorTemperature): Promise<void> {
...
export class FritzClient extends EventEmitter {
  ...
  public async getBulbs(): Promise<FritzBulb[]> {
  ...
  public async getButtons(): Promise<FritzButton[]> {
    return (await this.getDeviceInfos())
  ...
  public async invoke(method: string, args: Record<string, unknown> = {}): Promise<string> {
    const queryParams = {
      ...args,
      switchcmd: method,
      sid: this.sessionId,
